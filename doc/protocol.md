...
the server will reply with one of the following error messages:
 ...
 - "OUT_OF_MEMORY" The server cannot allocate enough memory for the job.
   The client should try again later.

 - "INTERNAL_ERROR" This indicates a bug in the server. It should never
   happen. If it does happen, please report it at
   http://groups.google.com/group/beanstalk-talk.

 - "BAD_FORMAT" The client sent a command line that was not well-formed.
   This can happen if the line does not end with , if non-numeric
 ...
 - "UNKNOWN_COMMAND" The client sent a command that the server does not
   know.
...
below).
 ...
 - `<bytes>` is an integer indicating the size of the job body, not including the
   trailing "". This value must be less than max-job-size (default: 2**16).
...
may be:
 ...
 - "INSERTED <id>" to indicate success.

   - `<id>` is the integer id of the new job

 - "BURIED <id>" if the server ran out of memory trying to grow the
 ...
 - "JOB_TOO_BIG" The client has requested to put a job with a body larger
   than max-job-size bytes.

 - "DRAINING" This means that the server has been put into "drain mode" and
...
The client then waits for one line of response, which may be:
 ...
 - "NOT_FOUND" if the job does not exist or is not either reserved by the
...
The client expects one line of response, which may be:
 ...
 - "RELEASED" to indicate success.

 - "BURIED" if the server ran out of memory trying to grow the priority
   queue data structure.

 - "NOT_FOUND" if the job does not exist or is not reserved by the client.
...
The reply is one of:
 ...
 - "WATCHING <count>" to indicate success.

   - `<count>` is the integer number of tubes currently in the watch list.

 - "NOT_IGNORED" if the client attempts to ignore the only tube in its
   watch list.
...
variations. All but the first operate only on the currently used tube.
 ...
 - "peek-ready" - return the next ready job.

 - "peek-delayed" - return the delayed job with the shortest delay left.

 - "peek-buried" - return the next job in the list of buried jobs.

There are two possible responses, either a single line:

 - "NOT_FOUND" if the requested job doesn't exist or there are no jobs in
   the requested state.
...
The response is one of:
 ...
 - "NOT_FOUND" if the job does not exist or is not in a kickable state. This
   can also happen upon internal errors.

 - "KICKED" when the operation succeeded.
...
The response is one of:
 ...
 - "NOT_FOUND" if the tube does not exist.
